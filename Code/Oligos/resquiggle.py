...
import numpy as np
np.seterr(all='raise')
## M.F. I set the seed for reproducibility
#
...
def _resquiggle_main(args):
    ...
    aligner = mappy.Aligner(
        # M.F.: I change the mapping parameters to work with short reads
        #
        # str(args.reference), preset=str('map-ont'), best_n=1)
        str(args.reference), k=int('9'), min_chain_score=int('5'), best_n=1)
    if not aligner:
        th.error_message_and_exit(
